...
-- Add user_id to inspections table for proper ownership tracking
ALTER TABLE public.inspections ADD COLUMN user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;

-- Update existing inspections to assign them to the admin user (if any exists)
