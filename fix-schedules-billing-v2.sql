-- FIX SCHEDULES AND BILLING TABLES - VERSION 2
...
-- Create NEW simplified RLS policies for schedules
...
-- Create NEW simplified RLS policies for invoices
...
-- Create NEW simplified RLS policies for invoice_services
...
BEGIN
  ...
  RAISE NOTICE '✅ Setup complete: % tables ready, % RLS policies active', table_count, policy_count;
  RAISE NOTICE '📋 Tables created/verified: schedules, invoices, invoice_services';
  RAISE NOTICE '🔒 RLS enabled with simplified policies for authenticated users';
  RAISE NOTICE '✨ You can now use the schedules and billing features!';
END $$;